...
import ExpandApproval from '../approval/expand';
import styles from './index.less';

const WithHistory = (props: any) => {
    const { columnConfig, rowData, cellRenderProps, value } = props;
    const { showHistory, addHistoryAuth, showApproval } = cellRenderProps || {};
    const Comp = config[columnConfig.columnType].detailComp;
    const cellData = value;
    const empty = !cellData || cellData.length === 0 || (cellData.length === 1 && !cellData[0].text && !cellData[0].value);
    return (
        <div className={styles.container}>
            <ExpandApproval record={rowData} showApproval={showApproval} />
            <div className={styles.cell}>{empty ? '-' : <Comp {...props} />}</div>
            <div
