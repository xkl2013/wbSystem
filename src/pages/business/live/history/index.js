...
export default class History extends React.Component {
    ...
    // 详情页面
    showDetailPage = async (params) => {
        let { tableId, rowId } = params || {};
        tableId = tableId || this.state.tableId;
        rowId = rowId || this.state.rowId;
        const data = await this.getInstanceDetail({ tableId, rowId });
        const rowData = Array.isArray(data.rowData) ? data.rowData : [];
        const formData = this.formatData(rowData);
        const helperData = await FormHelper.getFormData({ list: formData, rowLocked: data.isLocked });
    ...
    getColumnConfig = async ({ tableId }) => {
        ...
        if (response && response.success) {
            ...
            let columnConfig = _.cloneDeep(columns);
    ...
    addOrUpdateDataSource = async (value) => {
        ...
        if (response && response.success) {
            ...
            // 更新单条数据详情(最新操作的记录)
            this.showDetailPage({ tableId, rowId: response.data || rowId });
            // 更新跟进记录列表数据
    ...
    render() {
        ...
        const { maskClosable, noAdd, noEdit } = this.props;
        const isHideHistory = detailType === 'addPage' && initType === 'add';
        const plusScreen = document.body.clientWidth > 1920;
        return (
            visible && (
                <Modal
                    visible={visible}
                    ...
                    style={{ maxWidth: plusScreen ? '1536px' : '1152px' }}
                    width={isHideHistory ? '80%' : '60%'}
