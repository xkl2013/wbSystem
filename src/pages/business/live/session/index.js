...
const Session = (props) => {
    ...
    const config = businessConfig[38];
    const {
        tableId,
        getColumnConfig,
        getDataSource,
        ...
        columns.map((item) => {
            ...
            if (item.columnName === 'liveCode') {
                ...
                item.cellRenderProps = {
                    to: '/foreEnd/business/live/session/first',
                };
            }
            ...
            if (item.columnName === 'productSelectSecond') {
                item.renderDetailCell = renderDetailCell;
                item.cellRenderProps = {
                    to: '/foreEnd/business/live/session/second',
                };
            }
            if (item.columnName === 'productSelectFinal') {
                item.renderDetailCell = renderDetailCell;
                item.cellRenderProps = {
                    to: '/foreEnd/business/live/session/final',
                };
            }
            if (item.columnName === 'liveSort') {
                item.renderDetailCell = renderDetailCell;
                item.cellRenderProps = {
                    to: '/foreEnd/business/live/session/sorted',
                };
            }
            return item;
        });
    ...
    return (
        <>
            <ApolloTable
                ...
                cellEditable={checkPathname('/foreEnd/business/live/session/edit')}
                    ...
                    buttonsGroup: [
                        {
                            label: '新增场次',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/session/add'),
                            onClick: onClickAddSession,
                        },
                        {
                            label: '主播管理',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/session/talentManage'),
            ...
            <ModalDetail
                ...
                noEdit={!checkPathname('/foreEnd/business/live/session/edit')}
                noDel={!checkPathname('/foreEnd/business/live/session/del')}
                presetProcessor={[
                    {
                        type: 'confirm',
                        func: onConfirmEdit,
