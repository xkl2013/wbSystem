...
export const emitChangeCell4Msg = async ({ rowId, value }) => {
    const { tableId, addOrUpdateDataSource } = businessConfig[42];
...
export const formatColumns = (columns) => {
    const config = businessConfig[42];
    ...
    return (
        <>
                ...
                columns={useMemo(() => {
                    ...
                    if (firstItem) {
                        firstItem.cellRenderProps = {
                            ...
                            addHistoryAuth: checkPathname('/foreEnd/business/live/session/sorted/history/add'),
                        };
                    }
                    return formatColumns(columns);
                }, [columns])}
                dataSource={dataSource}
                cellEditable={checkPathname('/foreEnd/business/live/session/sorted/edit')}
                    ...
                    buttonsGroup: [
                        {
                            label: '新增',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/session/sorted/add'),
                            onClick: onClickAddSorted,
                        },
                        {
                            hidden: !checkPathname('/foreEnd/business/live/session/sorted/export'),
                            render: () => (
                                <ExportTable
                                    liveId={liveId}
                                    tableId={tableId}
                        ...
                        },
                        {
                            label: '删除',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/session/sorted/del'),
                            onClick: onClickDelLines,
                        },
                        {
                            label: '移至',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/session/sorted/transfer'),
            ...
            <HistoryDetail
                ...
                formatColumns={formatColumns}
                noAdd={!checkPathname('/foreEnd/business/live/session/sorted/history/add')}
                noEdit={!checkPathname('/foreEnd/business/live/session/sorted/history/edit')}
                noDel={!checkPathname('/foreEnd/business/live/session/sorted/history/del')}
            />
            <TransferProduct
