...
import { updateLineData, filterFormatColumns } from '@/pages/business/live/utils';
...
export const emitChangeCell4Msg = async ({ rowId, value }) => {
    const { tableId, addOrUpdateDataSource } = businessConfig[40];
...
export const formatColumns = (columns) => {
    const config = businessConfig[40];
    ...
    return (
        <>
                ...
                columns={useMemo(() => {
                    ...
                    if (firstItem) {
                        firstItem.cellRenderProps = {
                            ...
                            addHistoryAuth: checkPathname('/foreEnd/business/live/session/second/history/add'),
                        };
                    }
                    return formatColumns(columns);
                }, [columns])}
                dataSource={dataSource}
                cellEditable={checkPathname('/foreEnd/business/live/session/second/edit')}
                    ...
                    buttonsGroup: [
                        {
                            label: '新增',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/session/second/add'),
                            onClick: onClickAddSecond,
                        },
                        {
                            hidden: !checkPathname('/foreEnd/business/live/session/second/export'),
                            render: () => (
                                <ExportTable
                                    liveId={liveId}
                                    tableId={tableId}
                        ...
                        },
                        {
                            label: '删除',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/session/second/del'),
                            onClick: onClickDelLines,
                        },
                        {
                            label: '移至',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/session/second/transfer'),
            ...
            <HistoryDetail
                ...
                formatColumns={formatColumns}
                noAdd={!checkPathname('/foreEnd/business/live/session/second/history/add')}
                noEdit={!checkPathname('/foreEnd/business/live/session/second/history/edit')}
                noDel={!checkPathname('/foreEnd/business/live/session/second/history/del')}
            />
            <TransferProduct
