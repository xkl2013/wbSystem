...
export const emitChangeCell4Msg = async ({ rowId, value }) => {
    const { tableId, addOrUpdateDataSource } = businessConfig[41];
...
export const formatColumns = (columns) => {
    const config = businessConfig[41];
    ...
    return (
        <>
                ...
                columns={useMemo(() => {
                    ...
                    if (firstItem) {
                        firstItem.cellRenderProps = {
                            ...
                            addHistoryAuth: checkPathname('/foreEnd/business/live/session/final/history/add'),
                        };
                    }
                    return formatColumns(columns);
                }, [columns])}
                dataSource={dataSource}
                cellEditable={checkPathname('/foreEnd/business/live/session/final/edit')}
                    ...
                    buttonsGroup: [
                        {
                            label: '新增',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/session/final/add'),
                            onClick: onClickAddFinal,
                        },
                        {
                            hidden: !checkPathname('/foreEnd/business/live/session/final/export'),
                            render: () => (
                                <ExportTable
                                    liveId={liveId}
                                    tableId={tableId}
                        ...
                        },
                        {
                            label: '删除',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/session/final/del'),
                            onClick: onClickDelLines,
                        },
                        {
                            label: '移至',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/session/final/transfer'),
            ...
            <HistoryDetail
                ...
                formatColumns={formatColumns}
                noAdd={!checkPathname('/foreEnd/business/live/session/final/history/add')}
                noEdit={!checkPathname('/foreEnd/business/live/session/final/history/edit')}
                noDel={!checkPathname('/foreEnd/business/live/session/final/history/del')}
            />
            <TransferProduct
