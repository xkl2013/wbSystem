...
export const emitChangeCell4Msg = async ({ rowId, value }) => {
    const { tableId, addOrUpdateDataSource } = businessConfig[47];
    ...
    if (res && res.success) {
        ...
        // 修改状态为加入排序时，该字段不可编辑
        if (statusItem && String(statusItem.cellValueList[0].value) === '4') {
            statusItem.dynamicCellConfigDTO = { readonlyFlag: true, showDetailFlag: true };
        }
    }
    return res;
};
export const formatColumns = (columns) => {
    const config = businessConfig[47];
    ...
    return (
        <>
                ...
                columns={useMemo(() => {
                    ...
                    if (firstItem) {
                        firstItem.cellRenderProps = {
                            ...
                            addHistoryAuth: checkPathname('/foreEnd/business/live/bloggerSession/final/history/add'),
                        };
                    }
                    return formatColumns(columns);
                }, [columns])}
                dataSource={dataSource}
                cellEditable={checkPathname('/foreEnd/business/live/bloggerSession/final/edit')}
                    ...
                    buttonsGroup: [
                        {
                            label: '新增',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/bloggerSession/final/add'),
                            onClick: onClickAddFinal,
                        },
                        {
                            hidden: !checkPathname('/foreEnd/business/live/bloggerSession/final/export'),
                            render: () => (
                                <ExportTable
                                    liveId={liveId}
                                    tableId={tableId}
                        ...
                        },
                        {
                            label: '删除',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/bloggerSession/final/del'),
                            onClick: onClickDelLines,
                        },
                        {
                            label: '移至',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/bloggerSession/final/transfer'),
            ...
            <HistoryDetail
                ...
                formatColumns={formatColumns}
                noAdd={!checkPathname('/foreEnd/business/live/bloggerSession/final/history/add')}
                noEdit={!checkPathname('/foreEnd/business/live/bloggerSession/final/history/edit')}
                noDel={!checkPathname('/foreEnd/business/live/bloggerSession/final/history/del')}
            />
            <TransferProduct
