...
export const emitChangeCell4Msg = async ({ rowId, value }) => {
    const { tableId, addOrUpdateDataSource } = businessConfig[45];
...
export const formatColumns = (columns) => {
    const config = businessConfig[45];
    ...
    return (
        <>
                ...
                columns={useMemo(() => {
                    ...
                    if (firstItem) {
                        firstItem.cellRenderProps = {
                            ...
                            addHistoryAuth: checkPathname('/foreEnd/business/live/bloggerSession/first/history/add'),
                        };
                    }
                    return formatColumns(columns);
                }, [columns])}
                dataSource={dataSource}
                cellEditable={checkPathname('/foreEnd/business/live/bloggerSession/first/edit')}
                    ...
                    buttonsGroup: [
                        {
                            label: '新增',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/bloggerSession/first/add'),
                            onClick: onClickAddFirst,
                        },
                        {
                            hidden: !checkPathname('/foreEnd/business/live/bloggerSession/first/export'),
                            render: () => (
                                <ExportTable
                                    liveId={liveId}
                                    tableId={tableId}
                        ...
                        },
                        {
                            label: '删除',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/bloggerSession/first/del'),
                            onClick: onClickDelLines,
                        },
                        {
                            label: '移至',
                            type: 'primary',
                            hidden: !checkPathname('/foreEnd/business/live/bloggerSession/first/transfer'),
            ...
            <HistoryDetail
                ...
                formatColumns={formatColumns}
                noAdd={!checkPathname('/foreEnd/business/live/bloggerSession/first/history/add')}
                noEdit={!checkPathname('/foreEnd/business/live/bloggerSession/first/history/edit')}
                noDel={!checkPathname('/foreEnd/business/live/bloggerSession/first/history/del')}
            />
            <TransferProduct
