...
import { ModalTalent, TodayFollowFlag, WithHistory } from '@/pages/business/live/components';
...
export const filterFormatColumns = (columns, config) => {
    ...
    let history = 0;
    columns.map((item) => {
        if (!history && item.hideScope.indexOf('LIST') === -1) {
            item.renderDetailCell = () => {
                return { detailComp: WithHistory };
            };
            history = 1;
        }
        if (item.columnName === 'todayFollowFlag') {
            item.renderDetailCell = () => {
                return { detailComp: TodayFollowFlag };
            };
            item.renderDetailForm = () => {
                return { detailComp: TodayFollowFlag };
            };
        }
        if (item.columnName === 'commissionRate' || item.columnName === 'pitPrice') {
            item.renderDetailCell = () => {
                return { detailComp: ModalTalent };
            };
        }
        return item;
    });
