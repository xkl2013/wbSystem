...
class Index extends Component {
    ...
    render() {
        ...
        const labelWrap3 = LabelWrap.labelWrap3(formData);
        const { labelWrap4 } = LabelWrap;
        const { labelWrap5 } = LabelWrap;
        const { labelWrap7 } = LabelWrap;
        const labelWrap6 = LabelWrap.labelWrap6(formData);
        ...
        return (
            <div className={styles.detailPage}>
                ...
                {Number(formData.contractProjectType) !== 4 && (
                    <FlexDetail LabelWrap={labelWrap6} detail={formData} title="合作人信息" />
                )}
                <FlexDetail LabelWrap={[[]]} detail={formData} title="履约义务明细">
                    <BITable
                        rowKey="contractAppointmentId"
                        dataSource={formData.contractAppointmentList}
                        bordered
                        pagination={false}
                        columns={labelWrap3}
                    />
                ...
                {Number(formData.contractProjectType) !== 4 && (
                    <ChangeActor dataSource={formData} updateData={this.props.updataFn} />
                )}
