...
@connect(({ admin_approval, loading }) => {
    return {
        commerceApplyLists: admin_approval.commerceApplyLists,
        loading: loading.effects['admin_approval/getApplyCommerceLists'],
    };
})
// 合同审核申请列表 - 非商单审核
class CommonVerify extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            // pageDataLists: [],
    ...
    fetchData = (beforeFetch) => {
        ...
        this.props.dispatch({
            type: 'admin_approval/getApplyCommerceLists',
    ...
    render() {
        const { commerceApplyLists } = this.props;
