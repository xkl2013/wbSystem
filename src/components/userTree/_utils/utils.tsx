...
export const checkoutChildren = (data: any, value: any[] = []) => {
    let users: any = [];
    if (!data || data.length === 0) return;
    for (let i = 0; i < data.length; i++) {
        const obj: any = data[i];
        if (obj.symbol === 'user') {
            value.findIndex((ls: any) => String(ls.id) === String(obj.id)) < 0 && users.push(obj);
        }
        if (obj.children && obj.children.length > 0) {
            users = [...users, ...checkoutChildren(obj.children, value)];
        }
    }
    return users
}
